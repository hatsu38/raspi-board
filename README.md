...
視認距離 1m で快適に読める文字高は約 5mm（掲示物の経験則: 文字高 mm × 200 ≒ 距離）。1vh = 0.87mm なので、**実際にインクが乗る高さで 5.7vh 以上**が必要になる。

ここで font-size をそのまま文字高と見なすと足りない。字面は em を埋めきらないためで、Zen Maru Gothic Bold で実測した比率は次の通り。

| 文字種 | font-size に対する実インク高さ | 5mm に必要な font-size |
| --- | --- | --- |
| 漢字・かな | 約 0.89 em | **6.5vh** |
| 数字 | 約 0.72 em | **8.0vh** |

**数字は em の 7 割しか埋めない**ため、気温を天気 telop と同じ大きさに見せるには 1.2 倍の font-size が要る。

そのうえで、1080px に全部を 6.5vh 以上で並べることはできないので、表示は 2 層に分けている。

- **遠距離層**（1m から数秒で読む）— 時刻・今日の天気・今日の気温。`globals.css` の `.fs-clock` `.fs-today-telop` `.fs-today-temp`
- **近距離層**（近づいて読む）— 日付・降水確率・服装の文言・明日以降の天気。従来の `.fs-2xs`〜`.fs-2xl`

遠距離層のクラスが 5mm を割っていないかは `src/app/globals.test.ts` が `globals.css` を直接読んで検証している。遠くから読ませたい情報を増やすときは、同ファイルの `DISTANCE_READABLE` に足してから CSS を書く。
